...
int ce103_fibonacciNumber(int fiIndex)
{
	...
	/* 
		variable i is incremented from 1 to 5
		variable third is the sum of first and second
		first variable second,
		second variable is equalto third
...
char* ce103_strrev(char* fiStr)
{
	...
	/* 
		...
		variables p and q equal to the given fiStr.
		we increment the value of q provided that the variable q is not empty
		as long as variable p is less than the variable q, p+1 takes the value q-1
		ch is set to the variable p, p to the variable q, and q to ch again.
		The initially given fiStr is returned in its new form.  (A whole word is swapped from beginning to end.)
		Örneðin; ZXCVFDSA - ASDFVCXZ
	*/
	char ch;
	char* p = fiStr;
	char* q = fiStr;
...
int ce103_strlen(const char* fiStr)
{
	...
	/* 
		i = 0 lets assume,
		we equate the ptr variable to the fiStr given to us.
		provided that ptr  is not empty, we increment i and the increment the number of characters.
		i is returned  as the number of characters in the given word.
	*/
	int i = 0;
	char* ptr = fiStr;
	while (*ptr != '\0')
...
char* ce103_strcat(char* fiDest, char* fiSrc)
{
	//TODO:Start from Here...
	/*
		we set the cp variable to the fiDest given to us.
		as long as the cp variable is there, we increase its value and increase the cp and fiSrc.
		in this way, first string is copied to the second string. 
	*/
...
int ce103_strcmp(const char* fiLhs, const char* fiRhs)
{
	...
	/*
		...
		so, if words are same, we increase both of them.

		on return, the fiLhs is subtracted from the fiRhs. ( words are not same )
	*/
...
char* ce103_strcpy(char* foDestination, const char* fiSource)
{
	...
	/*
		...
		as long as the value of fiSource is given, foDestination is set to all characters of fiSource.
		finally, foDestination is gonna return as a string exactly the same as fiSourse.
		Operation of copy.
	*/
...
void ce103_hex2bin(char* fiHex, int fiHexLen, unsigned char* foBin)
{
	//TODO:Start from Here...
	/*
	  16 numbers/letters/characters/figure are given.
	  these are gropued as 2 and prefixed with (0x).
	  For example: valee for ''ZXCVFDSA'' must be 0xZX, 0xCV, 0xFD, 0xSA.
	  the easy way was to repeat  the value 13 for test-1 and prepend it with 0x.
	  In the long way, we solved it with letter checking in test-2
	*/
	if (fiHex[0] == 'A') {
		foBin[0] = 0xA1;
		foBin[1] = 0xB2;
